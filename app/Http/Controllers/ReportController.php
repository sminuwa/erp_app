...
class ReportController extends Controller
{
    ...
    //     // Extract budget year and quarter from date range
    //     $budget_year = date('Y', strtotime($from_date));
    //     $from_month = date('n', strtotime($from_date));
    //     $to_month = date('n', strtotime($to_date));

    //     // Determine quarter based on date range (take the quarter of from_date)
    //     $quarter = ceil($from_month / 3);

    //     // Preload aggregated budget per branch and category (budget is for quantity, not amount)
    //     $budgetQuery = DB::table('budgets')
    //         ->select('branch_id', 'category_id', DB::raw('SUM(month1 + month2 + month3) AS total_budget_qty'))
    //         ->where('budget_year', $budget_year)
    //         ->where('quarter', $quarter);

    //     // Apply branch filter to budget if specific branch selected
    //     if ($branch_id !== '%') {
    //         $budgetQuery->where('branch_id', $branch_id);
    //     }

    //     // Apply category filter to budget if specific categories selected
    //     if (!in_array('%', $category_id1)) {
    //         $budgetQuery->whereIn('category_id', $category_id1);
    //     }

    //     $budgetMap = $budgetQuery
    //         ->groupBy('branch_id', 'category_id')
    //         ->get()
    //         ->keyBy(fn($item) => $item->branch_id . '-' . $item->category_id);

    //     // Determine Grouping Options
    //     $group_by_category = $request->group_by_category ?? null;
    //     $group_by_product = $request->group_by_product ?? null;
    ...
    //     // Build the qty_available subquery with proper filtering
    //     $qtyAvailableSubquery = 'SELECT SUM(sp.qty_available)
    //     FROM store_products sp
    //     JOIN products p ON sp.product_id = p.id
    //     JOIN stores st ON sp.store_id = st.id
    //     WHERE p.category_id = categories.id';

    //     // Add branch filter to subquery if specific branch is selected
    //     if ($branch_id !== '%') {
    //         $qtyAvailableSubquery .= ' AND st.branch_id = ' . intval($branch_id);
    //     } else {
    //         // If no specific branch filter, still respect the company filter
    //         if ($company_id !== '%') {
    //             $qtyAvailableSubquery .= ' AND st.branch_id IN (
    //             SELECT b.id FROM branches b WHERE b.company_id = ' . intval($company_id) . '
    //         )';
    //         }
    //     }
    ...
    //             DB::raw('(' . $qtyAvailableSubquery . ') as qty_available')
    //         )
    ...
    //     }
    //     if ($branch_id !== '%') {
    //         $data->where('stores.branch_id', '=', $branch_id);
    //     }
    ...
    //     // Add branch details to select for budget comparison
    //     $groupFields[] = 'branches.id'; // Ensure branch is included for budget lookup
    ...
    //     // Add budget data to sales records (budget is for quantity)
    //     foreach ($sales as $record) {
    //         $key = $record->branch_id . '-' . $record->category_id;
    //         $budget = $budgetMap[$key] ?? null;
    //         $record->budget_qty = $budget ? $budget->total_budget_qty : 0;
    //         $record->achievement_percent = $record->budget_qty > 0
    //             ? round(($record->quantity / $record->budget_qty) * 100, 2)
    //             : null;
    //         $record->budget_variance_qty = $record->quantity - $record->budget_qty;
    //         $record->budget_variance_percent = $record->budget_qty > 0
    //             ? round((($record->quantity - $record->budget_qty) / $record->budget_qty) * 100, 2)
    //             : null;
    //     }
   ...
   public function loadCategorySaleReport(Request $request)
{
    ...
    // Handle branch_id - it can be an array or single value
    if ($branch_id == 'all' || $branch_id == '' || empty($branch_id)) {
        $branch_id = '%';
        $branch_ids = [];
    } else {
        // Ensure branch_id is an array
        $branch_ids = is_array($branch_id) ? $branch_id : [$branch_id];
    }

    // Store the actual branch_id value for subquery
    $branch_id_for_filter = $branch_id;
    ...
    // Preload aggregated budget per branch and category
    $budgetQuery = DB::table('budgets')
        ->select('branch_id', 'category_id', DB::raw('SUM(month1 + month2 + month3) AS total_budget_qty'))
        ->where('budget_year', $budget_year)
        ->where('quarter', $quarter);

    if ($branch_id != '%' && !empty($branch_ids)) {
        $budgetQuery->whereIn('branch_id', $branch_ids);
    }
    ...
    // FIXED: Proper qty_available subquery logic with multiple branch support
    if ($group_by_product) {
        // When grouping by product, get qty_available for SPECIFIC PRODUCT
        $qtyAvailableSubquery = 'SELECT COALESCE(SUM(sp2.qty_available), 0)
            FROM store_products sp2
            JOIN stores st2 ON sp2.store_id = st2.id
            WHERE sp2.product_id = products.id';
    } else {
        // When grouping by category only, get qty_available for ALL PRODUCTS in that category
        $qtyAvailableSubquery = 'SELECT COALESCE(SUM(sp2.qty_available), 0)
            FROM store_products sp2
    ...
    // FIXED: Proper branch filtering in subquery for multiple branches
    if ($branch_id_for_filter != '%' && !empty($branch_ids)) {
        $branch_ids_string = implode(',', array_map('intval', $branch_ids));
        $qtyAvailableSubquery .= ' AND st2.branch_id IN (' . $branch_ids_string . ')';
    } else {
        // If no specific branch filter, respect company filter
        if ($company_id != '%') {
            $qtyAvailableSubquery .= ' AND st2.branch_id IN (
                SELECT b2.id FROM branches b2 WHERE b2.company_id = ' . intval($company_id) . '
            )';
        }
    }
    ...
    // FIXED: Use stores.branch_id with multiple branch support
    if ($branch_id != '%' && !empty($branch_ids)) {
        $data->whereIn('stores.branch_id', $branch_ids);
    }
    ...
    \Log::info('Sales Data Sample:', [
        'total_records' => $sales->count(),
        'first_record' => $sales->first(),
        'branch_ids' => $branch_ids,
        'branch_id_for_filter' => $branch_id_for_filter,
        'qty_available_query' => $qtyAvailableSubquery
    ]);

    // Add budget data to sales records
    foreach ($sales as $record) {
        $key = $record->branch_id . '-' . $record->category_id;
        $budget = $budgetMap[$key] ?? null;
        $record->budget_qty = $budget ? $budget->total_budget_qty : 0;
    ...
    // Reset 'all' for readability in UI
    $category_id1_display = in_array('%', $category_id1) ? "all" : $category_id1;
    
    // Handle branch display for multiple branches
    if ($branch_id === '%') {
        $branch_id_display = "all";
    } else {
        $branch_id_display = is_array($branch_id) ? implode(', ', $branch_id) : $branch_id;
    }
    
    $company_id_display = $company_id === '%' ? "all" : $company_id;

    // Fetch additional company & branch info
    $company = ($company_id !== '%' && $company_id !== 'all') ? Company::find($company_id) : null;
    ...
    //     // Build the qty_available subquery with proper branch filtering
    //     $qtyAvailableSubquery = 'SELECT SUM(sp.qty_available) 
    //     FROM store_products sp 
    //     INNER JOIN stores st ON sp.store_id = st.id 
    //     WHERE sp.product_id = products.id';

    //     // Add branch filter to subquery if specific branches are selected
    //     if (!in_array('%', $branch_id)) {
    //         $branchIds = implode(',', array_map('intval', $branch_id));
    //         $qtyAvailableSubquery .= ' AND st.branch_id IN (' . $branchIds . ')';
    //     } else {
    //         // If no specific branch filter, still respect the company filter
    //         if ($company_id !== '%') {
    //             $qtyAvailableSubquery .= ' AND st.branch_id IN (
    //             SELECT b.id FROM branches b WHERE b.company_id LIKE "' . $company_id . '"
    //         )';
    //         }
    //     }
    ...
    //     if ($group_by_category) {
    //         $groupByColumns = ['branches.id', 'products.category_id']; // Branch > Category
    //     }

    //     if ($group_by_product) {
    //         $groupByColumns = ['branches.id', 'store_products.product_id']; // Branch > Product
    //     }
    ...
    public function loadCategorySaleBySiteReport(Request $request): View
    {
        ...
        // Apply branch filter to budget if specific branches selected
        if (!in_array('%', $branch_id)) {
            $budgetQuery->whereIn('branch_id', $branch_id);
        }
        ...
        return view(
            'pages.reports.sales_and_cash_analysis.load_sale_by_category_by_site_report',
            compact('salesByGroup', 'from_date', 'to_date', 'branch_id', 'category_id1', 'group_by_category', 'group_by_product', 'budget_year', 'quarter')
        );
    }
    ...
    public function loadRelationOfficerReport(Request $request)
    {
        ...
        $to_date = date('Y-m-d', strtotime($request->to_date));
        $company_id = $request->company_id;
        $budget_year = $request->input('budget_year', date('Y'));
        $quarter = $request->input('quarter');

        if (count($user_id) > 1)
            $user_id = array_filter($user_id, fn($val) => $val !== '%');
        if (count($category_id1) > 1)
            $category_id1 = array_filter($category_id1, fn($val) => $val !== '%');
        if ($company_id == 'all' || $company_id == '' || $company_id == null)
            $company_id = '%';

        // Preload aggregated budget per branch and category
        $budgetMap = DB::table('budgets')
            ->select('branch_id', 'category_id', DB::raw('SUM(month1 + month2 + month3) AS total_budget'))
            ->where('budget_year', $budget_year)
            ->where('quarter', $quarter)
            ->groupBy('branch_id', 'category_id')
            ->get()
        ...
        if ($is_summary) {
            ...
            if (!in_array('%', $category_id1))
                $data->whereIn('products.category_id', $category_id1);
            if (!in_array('%', $user_id))
                $data->whereIn('customers.relation_officer', $user_id);

            $salesByOfficer = $data
                ->groupBy(
                    'customers.relation_officer',
            ...
            foreach ($salesByOfficer as $ro_id => $salesGroup) {
        ...
        return view('pages.reports.sales_and_cash_analysis.load_relation_officer_report', compact(
            'salesByOfficer',
            'from_date',
            'to_date',
            'category_id1',
    ...
    public function loadCustomerDebtReport(Request $request)
    {
        ...
        // Base query for both debtors and creditors
        ...
        // Get debtors (positive balance, excluding zero)
        $debtorsQuery = clone $baseQuery;
        $debtors = $debtorsQuery->havingRaw('customers.opening_balance + COALESCE(SUM(general_account_ledgers.debit), 0) - COALESCE(SUM(general_account_ledgers.credit), 0) > 0')
            ->orderBy('due', 'desc')
            ->get()
            ->filter(function ($item) {
                return $item->due > 0; // Extra check to ensure no zero balances
            });

        // Get creditors (negative balance, excluding zero)
        $creditorsQuery = clone $baseQuery;
        $creditors = $creditorsQuery->havingRaw('customers.opening_balance + COALESCE(SUM(general_account_ledgers.debit), 0) - COALESCE(SUM(general_account_ledgers.credit), 0) < 0')
            ->orderBy('due', 'asc')
            ->get()
            ->filter(function ($item) {
                return $item->due < 0; // Extra check to ensure no zero balances
        ...
        // Map due to absolute value for creditors
        $creditors = $creditors->map(function ($item) {
            $item->due = abs($item->due);
            return $item;
        });
        ...
        return view(
            'pages.reports.customer_ledger_analysis.load_customer_total_debt_report',
            compact('debtors', 'creditors', 'from_date', 'to_date', 'company_id', 'branch_id', 'customer_id')
        );
    }
    ...
    public function loadAgeingReport(Request $request)
    {
        ...
        if ($company_id !== 'all' && !empty($company_id)) {
            $filters[] = ['branches.company_id', '=', $company_id];
        }
        if ($branch_id !== 'all' && !empty($branch_id)) {
            $filters[] = ['gal.branch_id', '=', $branch_id];
        }
        ...
        $sales = Customer::select(
            ...
            'users.name AS relation_officer',
            DB::raw('lt.last_transaction_date AS last_transaction_date'),
            DB::raw('DATEDIFF(NOW(), lt.last_transaction_date) AS age')
        )
            ->join('general_account_ledgers AS gal', 'gal.model_id', '=', 'customers.id')
            ->leftJoin('users', 'users.id', '=', 'customers.relation_officer')
            ->join('branches', 'branches.id', '=', 'gal.branch_id')
            ->joinSub($subquery, 'lt', function ($join) {
    ...
    public function loadAdditionalInvoiceReport(Request $request)
    {
        ...
        $sales = DB::table('purchase_expenses as pe')
            ->join('purchases as p', 'pe.purchase_id', '=', 'p.id')
            ->join('suppliers as s', 's.id', '=', 'pe.supplier_id')
            ...
            ->select(
                ...
                'p.purchase_date',
                'p.wbno',
                DB::raw('SUM(pe.amount) AS amount'),
                'pe.status',
                'p.created_at',
                'u.name as created_by'
            )
            ->whereBetween(DB::raw('DATE(p.purchase_date)'), [$from_date, $to_date])
            ->groupBy(
                'pe.reference',
                'p.id',
                's.name',
    ...
    //     $q = DB::table('purchase_expenses as pe')
    //         ->join('purchases as p', 'pe.purchase_id', '=', 'p.id')
    //         ->join('suppliers as s', 's.id', '=', 'pe.supplier_id')
    //         ->join('users as u', 'u.id', '=', 'p.created_by')
    //         ->join('branches as b', 'b.id', '=', 'p.branch_id')
    ...
    //     // Apply filters only when specified (avoid LIKE '%' on numeric ids)
    //     if (!empty($companyId) && $companyId !== 'all') {
    //         $q->where('b.company_id', $companyId);
    //     }
    //     if (!empty($branchId) && $branchId !== 'all') {
    //         $q->where('p.branch_id', $branchId);
    //     }
    //     if (!empty($supplierId) && $supplierId !== 'all') {
    //         $q->where('pe.supplier_id', $supplierId); // filter on pe (correct table)
    //     }
    //     if (!empty($status) && $status !== 'all') {
    //         $q->where('pe.status', $status); // filter on pe (correct table)
    //     }

    //     $sales = $q->get();

    //     $company = (!empty($companyId) && $companyId !== 'all') ? Company::find($companyId) : null;
    //     $branch = (!empty($branchId) && $branchId !== 'all') ? Branch::find($branchId) : null;
    ...
    public function loadAccountBalance(Request $request)
    {
        ...
        // Remove wildcard filters to allow index usage
        $branchCondition = !empty($branch_id) && $branch_id !== 'all' ? ['general_account_ledgers.branch_id' => $branch_id] : [];

        if ($type == "GeneralAccount") {
            $query = GeneralAccountLedger::select(
                ...
                ->whereDate('general_account_ledgers.date', '<=', $date)
        ...
        // Optimize filtering of zero balances
        if ($request->zeros == 'false') {
            $query->havingRaw('SUM(credit) <> SUM(debit)');
        }
    ...
    public function loadTrialBalance(Request $request)
    {
        ...
        // Calculate Previous Profit (before from_date)
        $previous_income = $this->generalAccountLedgerBy(null, $from_date, $company_id, $branch_id, 'GeneralAccount')
        ...
        $credit_sum2 = $query2->sum('credit');
        $debit_sum2 = $query2->sum('debit');
        $balance2 = $credit_sum2 - $debit_sum2 + $previous_profit;
        ...
        return view('pages.reports.ap_ar.trial_balance.load', compact(
            'ledger1',
            'ledger2',
            'ledger3',
            'ledger4',
    ...
    private function generalAccountLedgerBy($from_date, $to_date, $company_id, $branch_id, $type = null)
    {
        ...
        if ($type === 'Customer') {
            ...
            $query->join('suppliers', 'suppliers.id', '=', 'general_account_ledgers.model_id')
                ->where('general_account_ledgers.model_name', 'Supplier');
        } elseif ($type === 'GeneralAccount') {
            $query->leftJoin('general_accounts', 'general_accounts.id', '=', 'general_account_ledgers.model_id')
                ->where('general_account_ledgers.model_name', 'GeneralAccount');
        }
...
//        }
    ...
    public function loadRemittance(Request $request)
    {
        ...
        $ledgers = $query->select(
            ...
            ->where(function ($q) {
                $q->where('general_accounts.class', 'LIKE', 'A11%')
                    ...
                    ->orWhere('general_accounts.class', 'LIKE', 'A13%');
            })
            ->whereNotNull('general_account_ledgers.reference') // Ensure only valid transactions
            ->groupBy('general_account_ledgers.user_id', 'general_accounts.number')
            ->orderBy('general_accounts.number')
        ...
        return view('pages.reports.ap_ar.remittance.load', compact(
            'ledgers',
            'branch',
            'from_date',
            'to_date',
...
//            ->get();
    ...
    public function loadSupplierListReport(Request $request)
    {
        ...
        $suppliers = Supplier::select('suppliers.*')
            ->selectRaw('(SELECT SUM(CAST(gal.debit AS DECIMAL(15,2))) - SUM(CAST(gal.credit AS DECIMAL(15,2)))
                  FROM general_account_ledgers gal
                  WHERE gal.model_id = suppliers.id
                  AND gal.model_name = "Supplier") as balance')
    ...
    public function loadProducListReport(Request $request)
    {
        ...
        $records = Product::select('products.*')
            ...
            ->join('companies', 'branches.company_id', 'companies.id')
            ->where('branches.company_id', 'LIKE', $company_id)
            ->groupBy('products.id') // Ensure uniqueness
            ->orderBy('companies.name')
            ->orderBy('branches.code')
            ->orderBy('branches.name')
            ->get();
    ...
    public function loadSlowOverstayedReport(Request $request)
    {
        $validated = $request->validate([
            ...
            'report_type' => 'nullable|string', // Field for selecting report type
        ]);

        $company_id = $validated['company_id'];
        $branch_id = $validated['branch_id'];
        $report_type = $validated['report_type'] ?? 'all';
        ...
        // Overstayed products report (products not received recently but still in stock)
        if ($report_type == 'overstayed' || $report_type == 'all') {
            $overstayedInventory = $this->getOverstayedInventory($company_id, $branch_id);
            $inventory = $report_type == 'all' ? $inventory->concat($overstayedInventory) : $overstayedInventory;
        }

        // Slow-moving products report (products not sold recently but still in stock)
        if ($report_type == 'slow_moving' || $report_type == 'all') {
            $slowMovingInventory = $this->getSlowMovingInventory($company_id, $branch_id);
            $inventory = $report_type == 'all' ? $inventory->concat($slowMovingInventory) : $slowMovingInventory;
        }
    ...
    /**
     * Get overstayed inventory (products not received recently but still in stock)
     */
    ...
    //     // For each store product, find the latest purchase date
    ...
    //         // If we found a purchase and it's overstayed (30+ days), add to inventory
    //         if ($latestPurchase && $latestPurchase->days_since_received > 30) {
    //             $inventory->push($latestPurchase);
    //         } else if (!$latestPurchase) {
    //             // If no purchase record found, create a "never received" record
    ...
    public function loadBackdatedEntriesReport(Request $request)
    {
        ...
        // Include total column if available (for orders)
        elseif (Schema::hasColumn($table, 'total')) {
            $columns[] = "$table.total AS amount";
        }
        ...
        if (Schema::hasColumn($table, 'description')) {
            ...
            $columns[] = "$table.comment AS description";
        }
        if ($validated['type'] == "intersite_transfers") {
            $columns[] = "$table.vehicle_no AS description";
        }
        if ($validated['type'] == "purchases") {
            $columns[] = "$table.atc_no AS description";
        }
        if ($validated['type'] == "credit_notes" || $validated['type'] == "return_debits") {
            $columns[] = "$table.comment AS description";
        }
        ...
        $userColumns = ['posted_by', 'created_by', 'updated_by', 'sold_by'];
        $userColumn = null;
        ...
        // Select columns with fixed user name
        $userNameColumn = "users.name AS user_name";
        ...
        // Default branch to % if empty
        if ($validated['branch_id'] == '' || $validated['branch_id'] == 'all') {
            $validated['branch_id'] = '%';
        }

        // Filter by company if provided
        if (isset($validated['company_id']) && $validated['company_id'] != '' && $validated['company_id'] != 'all') {
            $query->where('branches.company_id', '=', $validated['company_id']);
        }
        ...
        $type = $validated['type'];
        return view('pages.reports.inventory.user_entries.load_backdated_postdated_entries_report', compact('reports', 'route_name', 'type'));
    }
    public function getRouteName($type)
    {
        ...
        switch ($type) {
            ...
            case 'purchase_expenses':
                $route_name = 'purchase.additional-invoice.print';
                break;
            default:
                $route_name = "orders.show";
        }
    ...
    {
        $users = DB::table('users')->where('active_status', 1)->get();
        return view('pages.reports.inventory.user_entries.user_entries_report', compact('users'));
    }
    ...
    public function loadUserEntriesReport(Request $request)
    {
        ...
        $userColumn = null;
        $possibleUserColumns = ['created_by', 'posted_by', 'updated_by', 'sold_by'];
        ...
        if ($branchColumn) {
            ...
            if ($company_id != '%') {
                $query->where('branches.company_id', '=', $company_id);
            }
        }
        ...
        // Determine user fields to select based on schema
        $userFields = [];
        $userFields[] = 'branches.name AS branch_name';
        $userFields[] = 'branches.code AS branch_code';

        // Add user fields only if user join is applied
        if ($userColumn) {
            $userFields[] = 'users.name AS user_name';
            // Add user_code field if it exists in the schema
            if (Schema::hasColumn('users', 'user_code')) {
                $userFields[] = 'users.user_code AS user_code';
            }
        }

        // Select final columns
        $query->select(array_merge($columns, $userFields));
        ...
        if (!empty($validated['created_by']) && $userColumn) {
            $created_by = $validated['created_by'];
            $query->where("$table.$userColumn", 'LIKE', "%{$created_by}%");
        }
        ...
        $route_name = $this->getRouteName($validated['type']);
        $type = $validated['type'];
        return view('pages.reports.inventory.user_entries.load_user_entries_report', compact('reports', 'route_name', 'type'));
    }

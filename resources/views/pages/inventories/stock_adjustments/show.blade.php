            ...
            <div class="container">
                <div class="row">
                    <div class="col-sm-12 text-right">
                        ...
                        <a class="btn btn-secondary btn-sm" target="_blank" href="{{ route('stock_adjustments.print', $record->id) }}">
                    ...
                    <div class="col-sm-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Stock Adjustment Details :: {{ $record->reference }} ({{ $record->status === 0 ? 'Pending' : 'Posted' }})</h4>
                            </div>
                            <div class="card-body">
                                <table class="table table-striped">
                                    ...
                                    <tr>
                                        <th>Branch</th>
                                        <td colspan="5">{{ ($record->branch->code ?? null).' - '.($record->branch->name ?? null) }}</td>
                                    </tr>
                                    <tr>
                                        <th>Date</th>
                                        <td>{{ optional($record->date)->toDayDateTimeString() }}</td>
                                        <th>Operation</th>
                                        <td >{!! $record->operation == 'in' ? '<span class="badge badge-success">In</span>' : '<span class="badge badge-danger">Out</span>' !!}</td>
                                        <th>Status</th>
                                        <td>{!! $record->status == 1 ? '<span class="badge badge-success">Posted</span>' : '<span class="badge badge-warning">Pending</span>' !!}</td>
                                    </tr>
                                    <tr>
                                        <th>Description</th>
                                ...
                                <table class="table table-striped"  style="font-size: 12px;">
                                    ...
                                    @foreach ($record->products as $product)
                                        <tr>
                                            ...
                                            <td class="text-right">{{ currency_sign().number_format($product->cost_price, 2) }}</td>
                                    ...
                                    <tfoot>
                                        <tr>
                                            ...
                                            <th style="text-align: right">{{ currency_sign().number_format($total, 2) }}</th>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>

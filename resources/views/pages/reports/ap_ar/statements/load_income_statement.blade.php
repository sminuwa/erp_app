...
{{--        <a href="{{ route('ajax.income.statement.report.print', [$from_month, $to_month, $income_year, $company_id, $branch_id, $category_id1, $category_id2]) }}"--}}
        <a href="{{ route('ajax.income.statement.report.print', [$from_date, $to_date, $company_id, $branch_id, $category_id1, $category_id2]) }}"
...
<table class="display table table-bordered caption" id="example1" data-ordering="true">
    <caption style="caption-size:top">
        ...
        {{--            INCOME STATEMENT FROM {{ $from_date == 'all' ? 'January' : monthName($from_month) }} AND--}}
        {{--            {{ $to_month == 'all' ? 'December' : monthName($to_month) }}--}}
        {{--        </h5>--}}
        <h5 style="text-align: center;">{{ strtoupper($branch->name ?? 'All Branches') }} <br>
            INCOME STATEMENT FROM {{ \Carbon\Carbon::parse($from_date)->format('F, Y') }} AND
            {{ \Carbon\Carbon::parse($to_date)->format('F, Y') }}
        </h5>
    </caption>
    <?php
    $total_revenue = 0;

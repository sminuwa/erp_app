...
<div class="table-responsive">
    <table class="display table table-bordered caption" id="example1">
        ...
        <tr>
            ...
            <th>R MARGIN</th>
            <th>W MARGIN</th>
            <th>MARGIN %</th> {{-- New column (Retail-based) --}}
        </tr>
        </thead>
        <tbody>
        @foreach ($stores as $store)
            @php
                ...
                // Retail margin % (row) = per-unit margin over retail price
                $margin_pct   = ($retail_price > 0) ? (($r_margin / $retail_price) * 100) : 0;
            ...
            <tr>
                ...
                <td style="text-align:right;">{{ number_format($margin_pct, 2) }}%</td> {{-- New --}}
            </tr>
        @endforeach
        </tbody>

        @php
            // Footer summary margin % (retail-based) = (Total Retail - Total Cost) / Total Retail
            $footer_margin_pct = ($sum_total_r > 0) ? (($sum_r_margin / $sum_total_r) * 100) : 0;
        @endphp
        ...
        <tr>
            ...
            {{-- Margin % (Retail-based) --}}
            <th style="text-align:right;">{{ number_format($footer_margin_pct, 2) }}%</th>
        </tr>
        </tfoot>
    </table>
</div>
